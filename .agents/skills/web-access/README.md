...
# 页面操作
curl -s -X POST --data-raw 'https://example.com' http://localhost:14456/new  # 新建 tab（v2.5.3 起 URL 走 POST body）
curl -s -X POST "http://localhost:14456/eval?target=ID" -d 'document.title'  # 执行 JS
curl -s -X POST "http://localhost:14456/click?target=ID" -d 'button.submit'  # JS 点击
curl -s -X POST "http://localhost:14456/clickAt?target=ID" -d '.upload-btn'  # 真实鼠标点击
curl -s -X POST "http://localhost:14456/setFiles?target=ID" \
  -d '{"selector":"input[type=file]","files":["/path/to/file.png"]}'        # 文件上传
curl -s "http://localhost:14456/screenshot?target=ID&file=/tmp/shot.png"     # 截图
curl -s "http://localhost:14456/scroll?target=ID&direction=bottom"           # 滚动
curl -s "http://localhost:14456/close?target=ID"                             # 关闭 tab
curl -s "http://localhost:14456/health"                                      # 查看状态（含 managedTabs 数量）
```
