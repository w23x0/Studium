...
健康检查，返回连接状态。
```bash
curl -s http://localhost:14456/health
```
...
curl -s -X POST --data-raw 'https://www.xiaohongshu.com/explore/xxx?xsec_source=app_share&xsec_token=ABC&type=normal' http://localhost:14456/new
```
...
关闭指定 tab。
```bash
curl -s "http://localhost:14456/close?target=TARGET_ID"
```
...
```bash
curl -s -X POST "http://localhost:14456/setFiles?target=ID" -d '{"selector":"input[type=file]","files":["/path/to/file1.png","/path/to/file2.png"]}'
```
